...
description: "Generates a clean, formal, ATS-compliant resume from an unstructured base resume. Features an 'Improve' mode to aggressively rewrite content for maximum professional impact, and a 'Tailor' mode to intelligently match ATS algorithms against a specific Job Description (JD). Outputs natively to PDF (default) or MS Word (.docx)."
---

# Resume Generator Skill

Use this skill whenever a user asks to "generate a new resume", "build me a resume", or similar requests involving translating a base resume into a formatted, ATS-friendly document. The skill operates in 3 distinct modes depending on the user's request:

1. **Standard Generation:** Quickly extracts raw resume text and compiles it into a beautiful, static ATS-compliant layout without altering the text meaning.
2. **Improvement Mode:** Triggered by **"improve my resume"**. Aggressively applies the Professional Content Improvement Rules to rewrite bullets, emphasize action verbs, standardize formatting, and natively bold prominent hard capabilities.
3. **JD Tailoring Mode:** Triggered by **"tailor my resume"** alongside a target Job Description. Translates the user's historical experience into the exact "Good" keywords demanded by the JD, deleting irrelevant noise.

**Output Format:** Output is always **PDF** by default. If the user explicitly requests Word format, you can output a **.docx**.

> **Note:** JD Tailoring Mode is a *superset* of Improvement Mode. When tailoring, ALL formatting rules from Step 2 (Keyword Bolding, Action Verbs, Metrics, etc.) are automatically applied in addition to the JD-specific translation logic.
...
2. **Professional Content Optimization (If requested)**:
   - If the user explicitly asks to **"improve my resume"**, you must take the extracted JSON and rewrite its contents generating a *new, optimized JSON* output using these Professional Content Improvement Rules:
...
3. **JD Tailoring Mode (If requested)**:
   - If the user provides a target Job Description (JD) and asks to "tailor" their resume, you will execute a highly advanced translation phase. **Note: All formatting/metric rules from Step 2 (Keyword Bolding, Action Verbs, Metrics) MUST still be applied during Tailoring!**
