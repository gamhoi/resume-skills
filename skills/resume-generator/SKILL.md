...
1. **Information Extraction**:
   ...
   - **If the input is a PDF** (especially one previously generated by this skill), you can use the `extract_resume.py` script to automatically extract the content into the standardized JSON schema with `<b>` keyword bolding tags preserved. See the Execution section for the command.
...
5. **Execution**:
   ...
   - **Extract from PDF (for re-editing an existing resume)**:
     - Command: `python3 <path_to_resume-generator>/scripts/extract_resume.py <path_to_input_pdf> <path_to_output>.json`
   - **Generate PDF (default)**:
     - Command: `python3 <path_to_resume-generator>/scripts/generate_resume.py <path_to_input_json> <path_to_output>.pdf`
   - **Generate Word / .docx (if specifically requested)**:
